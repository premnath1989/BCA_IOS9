...
-(NSDictionary *)searchSIOnlyResubmissionListingByName:(NSString *)SINO POName:(NSString *)poName Order:(NSString *)orderBy Method:(NSString *)method DateFrom:(NSString *)dateFrom DateTo:(NSString *)dateTo{
    ...
    if (([dateFrom length]>0)&&([dateTo length]>0)){
        ...
        [dateFromNew appendString:@" 00:00:00"];
        [dateToNew appendString:@" 24:00:00"];
        s = [database executeQuery:[NSString stringWithFormat:@"SELECT sim.*, po.ProductName,po.PO_Name,ifnull(sip.Sum_Assured,0) as Sum_Assured, po.QuickQuote FROM SI_Master sim left join SI_PO_Data po on sim.SINO=po.SINO left join SI_Premium sip on sim.SINO=sip.SINO where sim.SINO like \"%%%@%%\" and po.PO_Name like \"%%%@%%\" and sim.CreatedDate between \"%@\" and \"%@\" where sim.Resubmission=1 group by sim.ID order by %@ %@",SINO,poName,dateFromNew,dateToNew,orderBy,method]];
        NSLog(@"query %@",[NSString stringWithFormat:@"SELECT sim.*, po.ProductName,po.PO_Name,ifnull(sip.Sum_Assured,0) FROM SI_Master sim join SI_PO_Data po on sim.SINO=po.SINO join SI_Premium sip on sim.SINO=sip.SINO where sim.SINO like \"%%%@%%\" and po.PO_Name like \"%%%@%%\" and sim.CreatedDate between \"%@\" and \"%@\" where sim.Resubmission=1 group by sim.ID order by %@ %@",SINO,poName,dateFromNew,dateToNew,orderBy,method]);
    }
    else{
        s = [database executeQuery:[NSString stringWithFormat:@"SELECT sim.*, po.ProductName,po.PO_Name,ifnull(sip.Sum_Assured,0) as Sum_Assured, po.QuickQuote FROM SI_Master sim left join SI_PO_Data po on sim.SINO=po.SINO left join SI_Premium sip on sim.SINO=sip.SINO  where sim.SINO like \"%%%@%%\" and po.PO_Name like \"%%%@%%\" where sim.Resubmission=1 group by sim.ID order by %@ %@",SINO,poName,orderBy,method]];
    }
