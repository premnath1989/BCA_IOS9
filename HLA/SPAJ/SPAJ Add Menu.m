...
@interface SPAJAddMenu ()<PDFGenerationDelegate,PDFFunctionDelegate,SIListingDelegate,UIPopoverPresentationControllerDelegate>
...
@implementation SPAJAddMenu {
    ...
    -(void) addSignatureForPage9:(UIImage *)imgSignature1 UIImage2:(UIImage *)imgSignature2 UIImage3:(UIImage *)imgSignature3 UIImage4:(UIImage *)imgSignature4 onPDFData:(NSData *)pdfData {
        ...
        CGPDFDocumentRef pdf = CGPDFDocumentCreateWithProvider(provider);
        CGDataProviderRelease(provider);
        CGPDFPageRef page = CGPDFDocumentGetPage(pdf, 10);
        pageRect = CGPDFPageGetBoxRect(page, kCGPDFMediaBox);
        CGContextBeginPage(pdfContext, &pageRect);
        CGContextDrawPDFPage(pdfContext, page);
        
        // Draw the signature on pdfContext
        pageRect = CGRectMake(67, 476,96 , 53);
        CGImageRef pageImage1 = [imgSignature1 CGImage];
        CGContextDrawImage(pdfContext, pageRect, pageImage1);
        
        pageRect = CGRectMake(575, 476,96 , 53);
        CGImageRef pageImage4 = [imgSignature4 CGImage];
        CGContextDrawImage(pdfContext, pageRect, pageImage4);
    ...
    -(void)generateAllIMGPDF{
        ...
        if ([[dictionaryPOData valueForKey:@"RelWithLA"] isEqualToString:@"DIRI SENDIRI"]){
            ...
            arrayIMGName = [[NSMutableArray alloc]initWithArray:[modelSPAJHtml selectArrayHtmlFileName:@"SPAJHtmlName" SPAJSection:@"IMG_PH\",\"FRG_PH\",\"FRG_IN\",\"TP" SPAJID:[[dictTransaction valueForKey:@"SPAJID"] intValue]]];
            
            arrayIMGTranslateName = [[NSMutableArray alloc]initWithArray:[modelSPAJHtml selectArrayHtmlFileName:@"SPAJHtmlTranslateName" SPAJSection:@"IMG_PH\",\"FRG_PH\",\"FRG_IN\",\"TP" SPAJID:[[dictTransaction valueForKey:@"SPAJID"] intValue]]];
        }
        else{
    ...
    -(void)allImgSigned{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                UIAlertController *alertNoSPAJNumber = [alert alertInformation:@"Sukses" stringMessage:[NSString stringWithFormat:@"Nomor SPAJ anda adalah %lli",longSPAJNumber]];
                [self presentViewController:alertNoSPAJNumber animated:YES completion:^{
                    [_delegateSPAJMain actionGoToExistingList:nil];
