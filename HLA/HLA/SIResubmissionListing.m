...
static const int RESUBMIT_ALERT_TAG = 100;
static const int UPLOAD_DONE_ALERT_TAG = 101;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    UIAlertView* alert = [[UIAlertView alloc]initWithTitle:@"" message:[NSString stringWithFormat:@"Apakah anda ingin mengirimkan file SI Resubmission  %@?", [SINO objectAtIndex:indexPath.row]] delegate:self cancelButtonTitle:@"OK" otherButtonTitles: @"Batal", nil];
    alert.tag = RESUBMIT_ALERT_TAG;
    [alert show];
...
//        main.planName = [PlanName objectAtIndex:indexPath.row];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    } else if(alertView.tag == RESUBMIT_ALERT_TAG) {
        if(buttonIndex == 0) {
            [spinnerLoading startLoadingSpinner:self.view label:@"Upload sedang berjalan"];
            NSString *docsDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
            // We're going to put all the resubmission zip files at Documents/Resubmission directory
            NSString *resubmitDir = [docsDir stringByAppendingPathComponent:@"Resubmission"];
                        
            [self createDirectory:resubmitDir];
            
            // Prepare var for zipping
            NSString *planName = [[PlanName objectAtIndex:selectedId] stringByReplacingString:@" " withString:@""];
            NSString *ZipFileName = [NSString stringWithFormat:@"%@_%@_%@.zip", planName, [SINO objectAtIndex:selectedId], selectedSPAJ];
            ...
            if([SSZipArchive createZipFileAtPath:ZipPath withFilesAtPaths:paths]) {
                //upload to FTP
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self uploadResubmission:ZipFileName filePath:ZipPath];
                });
            }
        }
    } else if(alertView.tag == UPLOAD_DONE_ALERT_TAG) {
        [self dismissViewControllerAnimated:YES completion:nil];
    }
}
...
-(void)uploadResubmission:(NSString *)fileName filePath:(NSString *)filePath
{
...
//    NSString *preUploadParams = [NSString stringWithFormat:@"?agentID=%@",txtAgentCode.text];
    ...
    progressBar.TitleProgressBar = @"Uploading Resubmission";
    ...
    [progressBar.progressView setProgress: 0.5f animated:YES];
...
- (void) uploadisFinished {
    // Close the view if upload is sucessful
    ...
    [[session dataTaskWithURL:[NSURL URLWithString:urlStr]
            completionHandler:^(NSData *data,
                                ...
                                NSError *error)
    {
        //TODO: Add resubmission completed/failed handler
        if(error) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        UIAlertView* alert = [[UIAlertView alloc]initWithTitle:@"" message:[NSString stringWithFormat:@"Upload Berhasil"] delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
